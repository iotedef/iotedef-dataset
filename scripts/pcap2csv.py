...
def render_csv_row(pkt_sh, pkt_sc, fh_csv):
    ...
    try:
        ...
        if ether_pkt_sc.type != 0x800:
            logging.debug('Ignoring non-IP packet')
            return False
    except:
        logging.debug("Error in processing the packet")
        return False
    ...
    try:
        ...
        if proto == 17:
            ...
            logging.debug('Ignoring non-UDP/TCP packet')
            return False
    except:
        return False
...
def pcap2csv(in_pcap, out_csv):
    ...
    logging.debug('{} packets read, {} packets not written to CSV'.
